...
* `_shoppingCart` - кнопка, которая отображет корзину. При нажатии на кнопку вызывается событие `shoppingCart:open`
...
* `_button` - элемент открытия карточки
...
* `_adress` - элемент адреса доставки
...
* `_total` - общая сумма заказа, получаемая с сервера
