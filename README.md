...
* `_phone: string` - номер пользователя.
* `_items: IItem[] ` - категория лота.
* `_formErrors: IFormErrors` - цена лота.

##### Методы:
- `validateOrder(): void` - валидация заказа (проверка полей формы).
- `clearOrder(): void` - очистка полей формы.
- `set payment(value: IPaymentType)` - устанавливает способ оплаты.
...
- `set phone(value: string)` - устанавливает номер пользователя.
- `set items(value: IItem[])` - устанавливают категорию лота.
- `validatePayment(): void` - проверяет способ оплаты.
- `validateAddress(): void` - проверяет адрес пользователя.
- `validatePhone(): void` - проверяет номер пользователя.
- `validateEmail(): void` - проверяет почту пользователя.
- `postOrder(): void` - завершает заказ (делает событие order:post).
